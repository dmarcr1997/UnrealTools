...
#instantly delete assets or move to Trash
...
for asset in to_be_deleted:
    ...
    #instantly delete asset
    if instant_delete:
        deleted = editor_asset_lib.delete_loaded_asset(asset)

        if not deleted:
            unreal.log_warning("Asset {} not be deleted".format(asset_name))
            continue
        removed +=1
    #move assets to trash folder
        ...
        if not moved:
            unreal.log_warning("Asset {} not be moved to Trash".format(asset_name))
            continue
        removed += 1

output_test = "removed" if instant_delete else "moved to Trash folder"
unreal.log("{} of {} to be deleted assets, of {} selected, {}".format(removed, len(to_be_deleted), num_assets, output_test))
